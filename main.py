...
#background(transform.scale to scale the bg image to the screen size)
...
def main():
	...
	lost = False
	lost_count = 0	
	#Create a font to write text to the screen
	main_font = pygame.font.SysFont("comicsans", 50)
	lost_font = pygame.font.SysFont("comicsans", 70)

	#instantiate the player at given location
	...
	while run:
		#sets the game to run at 60 FPS regardless of performance
		...
		#check if there are no enemiesleft
		if len(enemies) == 0:
			level+=1
			wave_length += 5
			#spawn enemies
		...
		#down check if s key is pressed and set boundaries
